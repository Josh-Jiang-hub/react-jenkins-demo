...
const EmptyPlaceholder: VirtuosoProps['EmptyPlaceholder'] = ({ context }) => (
  <div>{!context.loaded ? 'Loading...' : 'Empty'}</div>
);
      ...
      setTimeout(() => {
        setMessageListData((current) => {
          ...
          const messages = Array.from({ length: 20 }, (_, i) =>
            createMessage(i % 3 === 0 ? currentUser : otherUser)
          );
          return {
            data: messages,
            scrollModifier: InitialDataScrollModifier,
  ...
  return (
    ...
    >
      <div>
        <button
          style={{
            fontWeight: currentChannel === 'channel-1' ? 'bold' : 'normal',
          }}
          onClick={() => switchChannel('channel-1')}
        >
          Channel 1
        </button>
        ...
        <button
          style={{
            fontWeight: currentChannel === 'channel-2' ? 'bold' : 'normal',
          }}
          onClick={() => switchChannel('channel-2')}
        >
          Channel 2
        </button>
        <button
          style={{
            fontWeight: currentChannel === 'channel-3' ? 'bold' : 'normal',
          }}
          onClick={() => switchChannel('channel-3')}
        >
          Channel 3
        </button>
      </div>
